...
matchCol = 10; %for original data
lureTypeCol = 11;
isAnswerCorrectCol = 13; %accuracy colums for for original data
isoMatchCol = 1; %match col for separate match and accu matrix
isoIsAnsCorCol = 2; %accu col for separate match and accu matrix
...
faW = nan(1,n(v)); %initiate row vector to save false alarm scores averaged across spans
...
%hits and false alarms per participant regardless of trials completed
[r c] = size(ptData); %size of original sub data
  incS = tSpans-(r/triPerSpan); %number of incomplete spans
nums = nan(r,isoIsAnsCorCol); %matrix to save match and accuracy columns
nums(:,isoMatchCol) = cell2mat(ptData(:,matchCol)); %save match labels separately
	nums(:,isoIsAnsCorCol) = double(cell2mat(ptData(:,isAnswerCorrectCol))); %save accuracy values as doubles instead of logical
hits = length(find(nums(:,isoMatchCol) == 1 & nums(:,isoIsAnsCorCol) == 1)); %hits: find tot correct (where match and accu cols = 1)
hPos = length(find(nums(:,isoMatchCol) == 1)); %hits: total match trials
hitR(1,pt) = hits/hPos; %hits: tot cor/tot match trials
if hitR(1,pt)==0 %avoids nan on inversion function
hitR(1,pt)=1/nTriFwR;
elseif hitR(1,pt)==1
hitR(1,pt)=(nTriFwR-1)/nTriFwR;
end %if hitR
fprintf(1, 'Vis: %i Task: %i Pt: %i Tot hits: %.2f, Tot hits pos: %.2f comp trials: %i # inc spans: %i\n', v, t, pt, hits, hPos, r, incS);
fa = length(find(nums(:,isoMatchCol) == 0 & nums(:,isoIsAnsCorCol) == 0)); %fa al: find total indices where match col = 0 and accu col = 0
faPos = length(find(nums(:,isoMatchCol) == 0)); %fa al: total nonmatch trials
faW(1,pt) = fa/faPos; %fa al: tot wrong on nonmatch trials/tot nonmatch trials
if faW(1,pt)==0 %avoids nan on inversion function
faW(1,pt)=1	/nTriFwR;
elseif faW(1,pt)==1
faW(1,pt)=(nTriFwR-1)/nTriFwR;
end %if faW
...
functionToEval = strcat(tasks{t}, '.', vision{v}, '.faAl', '= faW;'); %data eval will evaluate 
% run the text created in functionToEVal
 eval(char(functionToEval)); %save false alarms per task to structure
%adjusting matrix for overall percent correct scoring
if r < nTriFwR %when sub not reach max spans tested
accu = double(cell2mat(ptData(:,isAnswerCorrectCol))); %use for scoring accuracy
accu = [accu; .5*ones(nTriFwR-r,1)]; %scored correctly on half incomplete trials
...
accu = reshape(accu,triPerSpan,tSpans); %reorganize score column to sum scores per span
subPerCor(pt,:) = sum(accu)/triPerSpan; %each participant's percent correct per span saved to group matrix
...
%fprintf(1, 'Vis: %i Task: %i Pt: %i span: %i Tot cor match tri: %.2f, Tot match tri pos: %.2f comp trials: %i # inc spans: %i\n', v, t, pt, s, nansum(accu(naInd(s,:),s)), length(naInd), r, incS);
subPerCorLur(s,1:tSpans,pt) = [nansum(accu(naInd(s,:),s))/(length(naInd)-incS) nansum(accu(iChInd(s,:),s))/(length(iChInd)-incS) nansum(accu(sOOInd(s,:),s))/(length(sOOInd)-incS) nansum(accu(sTInd(s,:),s))/(length(sTInd)-incS)]; %percent correct per lure type and span per participant
...
dpRow(1,:) = norminv(hitR)-norminv(faW); %d prime per participant
